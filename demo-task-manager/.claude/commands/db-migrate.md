...
Manage database schema changes for Demo Task Manager safely and consistently.
...
pg_dump demo-task-manager > backup_$(date +%Y%m%d_%H%M%S).sql
...
psql demo-task-manager -c "\dt"  # List tables
psql demo-task-manager -c "\d users"  # Describe table
...
# Restore if needed
psql demo-task-manager < backup_TIMESTAMP.sql
...
Database migrations completed for Demo Task Manager! 📊
