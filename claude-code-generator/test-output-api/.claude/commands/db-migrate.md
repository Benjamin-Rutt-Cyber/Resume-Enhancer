...
Manage database schema changes for Test API safely and consistently.
...
pg_dump test-api > backup_$(date +%Y%m%d_%H%M%S).sql
...
psql test-api -c "\dt"  # List tables
psql test-api -c "\d users"  # Describe table
...
# Restore if needed
psql test-api < backup_TIMESTAMP.sql
...
Database migrations completed for Test API! 📊
