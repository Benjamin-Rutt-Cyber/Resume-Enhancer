...
Manage database schema changes for Test With Yes safely and consistently.
...
pg_dump test-with-yes > backup_$(date +%Y%m%d_%H%M%S).sql
...
psql test-with-yes -c "\dt"  # List tables
psql test-with-yes -c "\d users"  # Describe table
...
# Restore if needed
psql test-with-yes < backup_TIMESTAMP.sql
...
Database migrations completed for Test With Yes! 📊
