...
Deploy Test With Yes to production environment.
...
```bash
# Backup database
pg_dump test-with-yes_prod > backup_$(date +%Y%m%d_%H%M%S).sql
...
```bash
# Health check
curl https://test-with-yes.com/health

# API check
curl https://test-with-yes.com/api/v1/
...
psql test-with-yes_prod < backup_TIMESTAMP.sql
```
...
```bash
# Critical variables
DATABASE_URL=postgresql://user:pass@prod-db/test-with-yes
...
Deployment of Test With Yes completed!
