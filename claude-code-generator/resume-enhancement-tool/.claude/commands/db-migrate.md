...
Manage database schema changes for Resume Enhancement Tool safely and consistently.
...
pg_dump resume-enhancement-tool > backup_$(date +%Y%m%d_%H%M%S).sql
...
psql resume-enhancement-tool -c "\dt"  # List tables
psql resume-enhancement-tool -c "\d users"  # Describe table
...
# Restore if needed
psql resume-enhancement-tool < backup_TIMESTAMP.sql
...
Database migrations completed for Resume Enhancement Tool! 📊
